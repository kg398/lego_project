...
def decode_file(model):
    ...
    for z in range(0,len(model)):
        for y in range(0,16):
            for x in range(0,32):
                if model[z][y][x]!=0:
                    if model[z][y+3][x+1] == model[z][y][x]: #vertical brick
                        bricks.append({'x':x,'y':y,'z':z,'r':0,'px':x,'py':y+1})
                    if model[z][y+1][x+3] == model[z][y][x]: #horizontal brick
...
# master function for sorting brick list into a build que, quick sort(not always optimal)
